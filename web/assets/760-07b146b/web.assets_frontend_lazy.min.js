...
/* /odoo_saas_kit/static/src/js/subdomain_page.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('odoo_saas_kit.subdomain_page',function(require){"use strict";var rpc=require('web.rpc');var ajax=require('web.ajax');$('.get_subdomain_email').click(function(){var contract_id=$("#contract_id").attr('value');rpc.query({model:'saas.contract',method:'get_subdomain_email',args:[contract_id],}).then(function(){console.log("--Email sent--")});});$('.confirm_domain').click(function(){var subdomain_name=$("#subdomain_name").val();var contract_id=$("#contract_id").attr('value');if(subdomain_name.length==0)
$("#subdomain_name").css("border","1px solid #ff1414")
else{var domain_name=subdomain_name
$.blockUI({message:'<h1 style="color: #c9d0d6;"><i class="fa fa-spinner fa-spin" style="font-size:24px"></i>Please wait!</h1><center><p style="color: #f0f8ff;">We are creating your SaaS Instance.</p></center></div>',css:{'z-index':'1011','position':'fixed','padding':'0px','margin':'0px','width':'30%','top':'46%','left':'35%','text-align':'center','color':'rgb(0, 0, 0)','cursor':'wait','border':'none','background-color':'rgba(255, 255, 255, 0)',}});ajax.jsonRpc("/mail/confirm_domain",'call',{'domain_name':domain_name,'contract_id':contract_id}).then(function(return_dict){$.unblockUI();if(return_dict.status==1){$("#taken_warning").show();}else if(return_dict.status==2){$("#taken_warning").hide();location.href='/client/domain-created/redirect?contract_id='+contract_id}
else{$("#taken_warning").hide();$("#status_link").show();}});}});$('form').on('keyup keypress',function(e){var keyCode=e.keyCode||e.which;if(keyCode===13){e.preventDefault();return false;}});$('.go_to_account').click(function(ev){var contract_id=$(ev.currentTarget).val();rpc.query({model:'saas.contract',method:'redirect_invitation_url',args:[contract_id],}).then(function(url){console.log(url);location.href=url;});});$('.instance_login').click(function(ev){var client_id=parseInt($('.instance_login').attr('client_id'));rpc.query({model:'saas.client',method:'read',args:[[client_id],['client_url']],}).then(function(url){location.href=url[0]['client_url'];});});$('.revoke_domain').click(function(ev){var answer=confirm("Are You Sure You want to Revoke this domain..?");if(answer==true){var domain_id=parseInt($(ev.currentTarget).attr('domain_id'));rpc.query({model:'custom.domain',method:'revoke_subdomain_call',args:[domain_id],}).then(function(url){location.href=url});}});});;

/* /odoo_saas_kit/static/src/js/subdomain_operations.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('odoo_saas_kit.subdomain_operations',function(require){var rpc=require('web.rpc');$('.revoke_domain').click(function(ev){var answer=confirm("Are You Sure You want to Revoke this domain..?");if(answer==true){var domain_id=parseInt($(ev.currentTarget).attr('domain_id'));rpc.query({model:'custom.domain',method:'revoke_subdomain_call',args:[domain_id],}).then(function(url){location.href=url});}});$('#sub_domain_span_1, #add_domain_icon_div').click(function(ev){$("#add_custom_domain").modal("toggle");});$('#btn_add_domain').click(function(ev){var contract_id=$('#contract_id').attr('value');var domain_name=$('#add_subdomain_name').val();rpc.query({model:'custom.domain',method:'add_subdomain_call',args:[contract_id,domain_name],}).then(function(response){if(response.success){$("#add_custom_domain").modal("hide");location.href=response.msg;}else{$('#domain_taken_warning').text(response.msg);$('#domain_taken_warning').show();}});});});;

/* /odoo_saas_kit/static/src/js/portal_operations.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('odoo_saas_kit.portal_operations',function(require){var rpc=require('web.rpc');$('.instance_login').click(function(ev){var client_id=parseInt($('.instance_login').attr('client_id'));rpc.query({model:'saas.client',method:'read',args:[[client_id],['client_url']],}).then(function(url){location.href=url[0]['client_url'];});});});;

/* /odoo_saas_kit/static/src/js/user_pricing_modal.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('odoo_saas_kit.user_pricing_modal',function(require){"use strict";var rpc=require('web.rpc');var session=require('web.session');$('#modal_target').click(function(){if(!session.user_id){alert("Please login First to Continue !");return;}
var min_users=parseInt($('#min_user')).text;$('#new_min_user').attr('value',min_users);var product_id=parseInt($('.product_id').attr('value'));min_users=$('#new_min_user').val()
$('#total_cost').text('');rpc.query({model:'product.product',method:'read',args:[[product_id],['user_cost']],}).then(function(data){var total_amount=min_users*(parseInt(data[0]['user_cost']));$('#total_cost').text(total_amount);$("#modify_min_users").modal("toggle");});});$('#new_min_user').change(function(){var min_users=parseInt($('#min_user_quantity').attr('value'));var max_users=parseInt($('#max_user_quantity').attr('value'));var new_user=parseInt($('#new_min_user').val());var product_id=parseInt($('.product_id').attr('value'));if(new_user<min_users){alert("User must me more than or equal to "+min_users);}
else if(new_user>max_users){alert("User must me less than or equal to "+max_users);}else{rpc.query({model:'product.product',method:'read',args:[[product_id],['user_cost']],}).then(function(data){var total_amount=new_user*parseFloat(data[0]['user_cost']);$('#total_cost').text(total_amount);});}});$('#min_user_submit').click(function(){var min_users=parseInt($('#min_user_quantity').attr('value'));var max_users=parseInt($('#max_user_quantity').attr('value'));var new_user=parseInt($('#new_min_user').val());if(new_user<min_users){alert("User must me more than or equal to "+min_users);}
else if(new_user>max_users){alert("User must me less than or equal to "+max_users);}else{$('#min_user').text(new_user);$('#number_of_user').attr('value',new_user);$('#modify_min_users').modal('hide');}});});;

/* /odoo_saas_kit/static/src/js/user_pricing.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('odoo_saas_kit.user_pricing',function(require){"use strict";const publicWidget=require('web.public.widget');const wUtils=require('website.utils');require('website_sale.website_sale');publicWidget.registry.WebsiteSale.include({_submitForm:function(){const params=this.rootProduct;const $product=$('#product_detail');const productTrackingInfo=$product.data('product-tracking-info');params.number_of_user=this.$form.find('#number_of_user').val();if(productTrackingInfo){productTrackingInfo.quantity=params.quantity;$product.trigger('add_to_cart_event',[productTrackingInfo]);}
params.add_qty=params.quantity;params.product_custom_attribute_values=JSON.stringify(params.product_custom_attribute_values);params.no_variant_attribute_values=JSON.stringify(params.no_variant_attribute_values);return this.addToCart(params);},});});;

/* /saas_kit_custom_plans/static/src/js/custom_plan.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('saas_kit_custom_plan.product_page',function(require){"use strict";var session=require('web.session');var rpc=require('web.rpc');var ajax=require('web.ajax');var number=1;var publicWidget=require('web.public.widget');var total_app_value_span=0;var final_cost_value_span=0;var users_price=0;var prev_cycle='Monthly';var remove_button=false;var apps=new Array();var price_dict={};var max_users=0;var max_no_users=false;var annual_discount=false;var discount_percent=0;var discount_value_span=0;publicWidget.registry.CustomPlan=publicWidget.Widget.extend({selector:'.custom_plan_section',events:{'click #toggle_input':'_onToggleCategoryView','click  .select_button':'_onSelectApp','mouseenter .selected_button':'_onEnterSelectedButton','mouseleave .remove_button':'_onLeaveRemoveButton','click  .remove_button':'_onClickRemoveButton','click   #view_more_apps':'_onClickMoreApps','click  .remove_app_button':'_onClickRemoveAppButton','change #number_of_users':'_onChangeUsers','click #buy_now':'_onClickBuyNow','click .version_button':'_onSelectVersion','click .billing_cycle':'_onSelectBilling',},_return_apps_data:function(ev){return apps;},_return_apps_value:function(ev){return total_app_value_span;},_get_price_dict:function(ev){return price_dict;},_check_discount:function(){if(annual_discount){if(number==12){discount_value_span=(final_cost_value_span*(discount_percent/100.00));console.log(discount_value_span);final_cost_value_span=final_cost_value_span-discount_value_span;}
else{discount_value_span=0;}}
return true;},_check_if_discounted:function(cost){var discounted_cost;if(annual_discount&&(number==12)){discounted_cost=cost-(cost*(discount_percent/100));return discounted_cost;}
else return cost;},_onToggleCategoryView:function(ev){var version_code=$('#dropdownmenu2').attr('data-code');if($('input').is(':checked')){$.get("/show/categ/view",{}).then(function(data){$('#normal_view_main_div').replaceWith(data);});}
else{$.get("/show/normal/view",{}).then(function(data){$('#category_view_main_div').replaceWith(data);});}
$.get("/show/selected/apps/view",{}).then(function(data){$('#right_block').replaceWith(data);apps.length=0;total_app_value_span=0;final_cost_value_span=0;users_price=0;});},_onSelectVersion:function(event){var version_name=$(event.target).text();var version_code=$(event.target).attr('data-code');$('#dropdownmenu2').text(version_name);$('#dropdownmenu2').attr('data-code',version_code);},update_costing_view:function(price_1){var self=this;total_app_value_span=total_app_value_span+price_1;final_cost_value_span=total_app_value_span+users_price;self._check_discount();$('#total_app_value_span').text(total_app_value_span.toString()+' ');$('#discount_value_span').text(discount_value_span.toString()+' ');$('#final_cost_value_span').text(final_cost_value_span.toString()+' ');$('#pay_now_value_span').text(final_cost_value_span.toString()+' ');},_onSelectBilling:function(event){var self=this;var billing_type=$(event.target).text();$('#dropdownmenu3').text(billing_type);rpc.query({model:'saas.odoo.version',method:'get_default_saas_values',args:[],}).then(function(data){if(billing_type=='Monthly'){number=1;if(prev_cycle=='Yearly'){total_app_value_span=total_app_value_span/12;}
prev_cycle='Monthly';$('#discount').css('display','none');}
else{number=12;annual_discount=data['annual_discount'];discount_percent=data['discount_percent'];if(prev_cycle=='Monthly'){total_app_value_span=total_app_value_span*12;}
prev_cycle='Yearly';$('#discount').css('display','flex');}
if(data['is_users']){var user_cost=data['user_cost'];var users=$('#number_of_users').val();if(data['is_free_users']){users_price=(users-data['free_users'])*user_cost;if(users_price<0){users_price=0;}}else{users_price=users*user_cost;}
users_price=users_price*number;$('#user_price_span').text(user_cost.toString()+' ');$('#total_users_value_span').text(users_price.toString()+' ');}
final_cost_value_span=total_app_value_span+users_price;self._check_discount();$('#total_app_value_span').text(total_app_value_span.toString()+' ');$('#discount_value_span').text(discount_value_span.toString()+' ');$('#final_cost_value_span').text(final_cost_value_span.toString()+' ');$('#pay_now_value_span').text(final_cost_value_span.toString()+' ');});},change_app_card:function(ev){$(ev.currentTarget).closest('.select_button_div').find('.select_button').css('display','none');$(ev.currentTarget).closest('.select_button_div').find('.selected_button').css('display','inline');$(ev.currentTarget).closest('.app_card').css({'background-color':'#3AADAA;','border':'1px solid #3AADAA'});$(ev.currentTarget).closest('.col-8').find('.app_name').css({'color':'#FFFFFF;'});$(ev.currentTarget).closest('.col-8').find('.span_price').css({'color':'#FFFFFF;'});$(ev.currentTarget).closest('.col-8').find('.price').css({'color':'#FFFFFF;'});},set_total_count:function(ev){var total_count=$('#total_apps_count').text();total_count=parseInt(total_count);total_count=total_count+1;if(total_count<10){$('#total_apps_count').text('0'+total_count.toString());}else{$('#total_apps_count').text(total_count.toString());}},update_side_portal_height:function(increase){var height=$('#line_complete').css('height');height=height.split('p');if(height){height=parseInt(height[0]);if(increase){height=height+37;}else{height=height-37;}
$('#line_complete').css('height',height.toString()+'px');}},get_user_price:function(data){var user_cost=data['user_cost'];if(data['is_free_users']){var users=$('#number_of_users').val();users_price=(users-data['free_users'])*user_cost;if(users_price<0){users_price=0;}}else{users_price=users*user_cost;}
users_price=users_price*number;},add_side_bar_app:function(technical_name,name,price,currency){$('#test_apps_detail_div').prepend('<div class="apps_name_head d-flex" id="'+technical_name+'">'+'<div class="nodes">'+'</div>'+'<div class="node_head">'+'</div>'+'<div class="app_name_detail">'+name+'</div>'+'<div class="app_price"> '+price+' '+currency+'</div>'+'<div class="remove_button_div">'+'<button class="remove_app_button">'+'REMOVE'+'</button>'+'</div>'+'</div>');},_onSelectApp:function(ev){var self=this;if(!session.user_id){alert("Please login First to Continue !");return;}
var technical_name=$(ev.currentTarget).closest('.app_card').find('.app_tech_name').text();var name=$(ev.currentTarget).closest('.app_card').find('span.app_name').text();var currency=$('#currency').text();rpc.query({model:'saas.module',method:'search_read',args:[[['technical_name','=',technical_name]],['price']],}).then(function(data){var price=parseInt(data[0]['price']);price_dict[technical_name]=price
var price_1=price*number;rpc.query({model:'saas.odoo.version',method:'get_default_saas_values',args:[],}).then(function(data){if(data['is_user']){self.get_user_price(data)}
$('#apps_complete_details').hide();self.set_total_count(ev);self.change_app_card(ev);self.update_side_portal_height(true);self.update_costing_view(price_1);self.add_side_bar_app(technical_name,name,price,currency);apps.push(technical_name);});});},_onEnterSelectedButton:function(ev){$(ev.currentTarget).closest('.select_button_div').find('.selected_button').css('display','none');$(ev.currentTarget).closest('.select_button_div').find('.remove_button').css('display','inline');},_onLeaveRemoveButton:function(ev){if(remove_button){remove_button=false;}else{$(ev.currentTarget).closest('.select_button_div').find('.selected_button').css('display','inline');$(ev.currentTarget).closest('.select_button_div').find('.remove_button').css('display','none');}},decrease_app_count:function(ev){var total_count=$('#total_apps_count').text();total_count=parseInt(total_count);total_count=total_count-1;if(total_count<10){$('#total_apps_count').text('0'+total_count.toString());}else{$('#total_apps_count').text(total_count.toString());}},remote_from_side_panel:function(ev){$(ev.currentTarget).closest('.select_button_div').find('.selected_button').css('display','none');$(ev.currentTarget).closest('.select_button_div').find('.remove_button').css('display','none');$(ev.currentTarget).closest('.select_button_div').find('.selected_button').css('display','none');$(ev.currentTarget).closest('.select_button_div').find('.select_button').css('display','inline');$(ev.currentTarget).closest('.app_card').css({'background-color':'#FFFFFF;','border':'1px solid #CCCCC'});$(ev.currentTarget).closest('.app_card').css('border','1px solid #CCCCCC');$(ev.currentTarget).closest('.col-8').find('.app_name').css({'color':'#000000;'});$(ev.currentTarget).closest('.col-8').find('.price').css({'color':'#000000;'});$(ev.currentTarget).closest('.col-8').find('.span_price').css({'color':'#000000;'});},_onClickRemoveButton:function(ev){var self=this;self.decrease_app_count()
self.remote_from_side_panel(ev)
remove_button=true;var technical_name=$(ev.currentTarget).closest('.app_card').find('.app_tech_name').text();$('#'+technical_name).remove();$('#'+technical_name+'_node').remove();self.update_side_portal_height(false)
var price=price_dict[technical_name]
price=price*number;total_app_value_span=total_app_value_span-price;final_cost_value_span=total_app_value_span+users_price;self._check_discount();$('#total_app_value_span').text(total_app_value_span.toString()+' ');$('#discount_value_span').text(discount_value_span.toString()+' ');$('#final_cost_value_span').text(final_cost_value_span.toString()+' ');$('#pay_now_value_span').text(final_cost_value_span.toString()+' ');apps.splice($.inArray(technical_name,apps),1);},remove_app_card:function(technical_name){$('#'+technical_name+'_node').remove();$('#'+technical_name).remove();$('#'+technical_name+'_main').find();$('#'+technical_name+'_main').find('.selected_button').css('display','none');$('#'+technical_name+'_main').find('.remove_button').css('display','none');$('#'+technical_name+'_main').find('.selected_button').css('display','none');$('#'+technical_name+'_main').find('.select_button').css('display','inline');$('#'+technical_name+'_main').css({'background-color':'#FFFFFF;','border':'1px solid #CCCCC'});$('#'+technical_name+'_main').css('border','1px solid #CCCCCC');$('#'+technical_name+'_main').find('.app_name').css({'color':'#000000;'});$('#'+technical_name+'_main').find('.price').css({'color':'#000000;'});$('#'+technical_name+'_main').find('.span_price').css({'color':'#000000;'});},_onClickRemoveAppButton:function(ev){var self=this;self.decrease_app_count();var technical_name=$(ev.currentTarget).closest('.apps_name_head').attr('id');self.remove_app_card(technical_name)
self.update_side_portal_height(false)
var price=price_dict[technical_name]
price=price*number;total_app_value_span=total_app_value_span-price;final_cost_value_span=total_app_value_span+users_price;self._check_discount();$('#total_app_value_span').text(total_app_value_span.toString()+' ');$('#discount_value_span').text(discount_value_span.toString()+' ');$('#final_cost_value_span').text(final_cost_value_span.toString()+' ');$('#pay_now_value_span').text(final_cost_value_span.toString()+' ');apps.splice($.inArray(technical_name,apps),1);},_onClickMoreApps:function(ev){$('#apps_details').attr('style','visiblity: hidden');$('#apps_details').attr('style','height: 0px');$('#apps_complete_details').attr('style','visiblity: visible');$('#apps_complete_details').attr('style','height: auto');},_onChangeUsers:function(e){var self=this;var users=$('#number_of_users').val();if(max_users!==0&&users==max_users+1){alert("You cannot add more users.\nMaximum number of user reached!");$('#number_of_users').val(max_users);return;}
rpc.query({model:'saas.odoo.version',method:'get_default_saas_values',args:[],}).then(function(data){var user_cost=data['user_cost'];if(max_users==0){max_users=data['max_users'];if(users==max_users+1){alert("You cannot add more users.\nMaximum number of user reached!");$('#number_of_users').val(max_users);return;}}
if(data['is_free_users']){users_price=(users-data['free_users'])*user_cost;if(users_price<0){users_price=0;}}else{users_price=users*user_cost;}
users_price=users_price*number
$('#user_price_span').text(user_cost.toString());$('#total_users_value_span').text(users_price.toString()+' ');final_cost_value_span=users_price+total_app_value_span;self._check_discount();$('#discount_value_span').text(discount_value_span.toString()+' ');$('#final_cost_value_span').text(final_cost_value_span.toString()+' ');$('#pay_now_value_span').text(final_cost_value_span.toString()+' ');});},_onClickBuyNow:function(ev){var self=this;var billing_type=$('#dropdownmenu3').text().trim();if(!billing_type){alert("Please select Billing Cycle");return;}
if(apps.length==0){alert("Please select Atleast one App..");return;}
var recurring_interval=0;if(billing_type=='Monthly'){recurring_interval=1;}
else{recurring_interval=12;}
var users=$('#number_of_users');var version_code=$('#dropdownmenu2').attr('data-code');var version_name=$('#dropdownmenu2').text().trim();var number_of_users=parseInt(users.val());if(users.length&&!number_of_users){alert('Please Enter User to Continue');}
rpc.query({model:'saas.odoo.version',method:'get_default_saas_values',args:[],}).then(function(data){var user_cost=data['user_cost'];var total_cost=total_app_value_span;total_cost=self._check_if_discounted(total_cost);if(data['is_free_users']){user_cost=(number_of_users-data['free_users'])*user_cost*number;if(user_cost<0){user_cost=0;}
user_cost=self._check_if_discounted(user_cost);}
else{user_cost=user_cost*number_of_users*number;user_cost=self._check_if_discounted(user_cost);}
ajax.jsonRpc("/saas/add/plan",'call',{'apps':apps,'saas_users':number_of_users,'version_code':version_code,'version_name':version_name,'total_cost':total_cost,'users_cost':user_cost,'recurring_interval':recurring_interval,}).then(function(a){location.href='/shop/cart';});});},});});;

/* /saas_kit_custom_plans/static/src/js/update_app.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('saas_kit_custom_plan.update_app',function(require){var rpc=require('web.rpc');var apps=new Array();$('#add_apps_icon_div, #add_app_span_1').click(function(){var contract_id=parseInt($('#add_apps_submit').attr('value'));rpc.query({model:'saas.contract',method:'get_module',args:[contract_id],}).then(function(data){if(data){apps.length=0;$('.apps_to_select_button').css('display','inline');$('.apps_selected_button').css('display','none');$('.apps_to_add_data_row').css('background','#FFFFFF');$("#add_apps").modal("toggle");}});});$('#add_apps_submit').click(function(){if(apps.length==0){alert('Please Select Atleast One App to continue !')
return}
var contract_id=parseInt($('#add_apps_submit').attr('value'));rpc.query({model:'saas.contract',method:'add_apps',args:[apps,contract_id],}).then(function(data){$('#add_apps').modal('hide');location.href='/my/saas/contract/'+contract_id+'?access_token='+data});});$('.apps_to_select_button').click(function(ev){var technical_name=$(ev.currentTarget).closest('.apps_to_add_img_row').find('.apps_to_add_tech_name').text();apps.push(technical_name);$(ev.currentTarget).closest('.apps_to_add_data_row').css('background','rgba(141, 255, 87, 0.2)');$(ev.currentTarget).closest('.apps_to_add_img_row').find('.apps_selected_button').css('display','inline');$(ev.currentTarget).closest('.apps_to_add_img_row').find('.apps_to_select_button').css('display','none');});$('.apps_selected_button').click(function(ev){var technical_name=$(ev.currentTarget).closest('.apps_to_add_img_row').find('.apps_to_add_tech_name').text();apps.splice($.inArray(technical_name,apps),1);$(ev.currentTarget).closest('.apps_to_add_data_row').css('background','#FFFFFF');$(ev.currentTarget).closest('.apps_to_add_img_row').find('.apps_selected_button').css('display','none');$(ev.currentTarget).closest('.apps_to_add_img_row').find('.apps_to_select_button').css('display','inline');});});;
...
/* /saas_kit_backup/static/src/js/backup_process.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('wk_backup_restore.backup_process',function(require){"use strict";var publicWidget=require('web.public.widget');var rpc=require('web.rpc');var update_req=false;publicWidget.registry.BackupProcess=publicWidget.Widget.extend({selector:'.backup_process_main',events:{'click #create_backup_process':'_onClickBackupProcessButton','click .fre_cyc_button':'_onClickFreqCycleButton','click #create_process_button':'_onClickCreateProcessButton','click #update_backup_process':'_onClickUpdateProcessButton','click #cancel_backup_process':'_onClickCancelBackupButton','click .download':'_onClickDownloadButton',},_onClickBackupProcessButton:function(ev){$("#create_backup_process_model").modal("toggle");},_onClickFreqCycleButton:function(ev){var freq_cycle_text=$(event.target).text();var freq_cycle_code=$(event.target).attr('data-code');$('#btn_frq_cycle_drpdwn').text(freq_cycle_text);$('#btn_frq_cycle_drpdwn').attr('data-code',freq_cycle_code);},_onClickCreateProcessButton:function(ev){var freq=1;var freq_cycle_code=$('#btn_frq_cycle_drpdwn').attr('data-code');if(freq_cycle_code=='half_day'){freq=2;}
var starting_date=$('#date_time_input').val();if(!starting_date){alert('Please Enter Starting Date and Time...')
return;}
var client_id=parseInt($('#create_process_button').attr('value'));rpc.query({model:'saas.client',method:'get_create_backup_process_data',args:[freq,freq_cycle_code,starting_date,update_req,client_id],}).then(function(data){update_req=false;location.href='/my/backup/files/'+data['contract_id']+'?access_token='+data['token'];});},_onClickUpdateProcessButton:function(ev){update_req=true;$("#create_backup_process_model").modal("toggle");},_onClickCancelBackupButton:function(ev){var answer=confirm("Are You Sure You want to cancel the Backup Process..?");if(answer==true){var client_id=parseInt($('#cancel_backup_process').attr('value'));rpc.query({model:'saas.client',method:'get_cancel_backup_process_call',args:['self',client_id],}).then(function(data){location.href='/my/backup/files';});}},_onClickDownloadButton:function(ev){var client_id=$(ev.currentTarget).attr('client_id');var detail_id=$(ev.currentTarget).attr('detail_id');var name=$(ev.currentTarget).closest('.tbody_row').find('.name_text').text();rpc.query({model:'saas.client',method:'get_download_backup_zip_call',args:['self',parseInt(client_id),parseInt(detail_id),name],}).then(function(data){if(data){location.href=data;}
else{alert('Authentication Failed...');}});},});});;

/* /saas_kit_trial/static/src/js/saas_trial.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('odoo_saas_kit_trial.saas_trial',function(require){"user strict";var rpc=require('web.rpc');var ajax=require('web.ajax');$('.get_trial').on('click',function(event){var product_id=$(".product_id").attr('value');var quantity=$(".quantity").val();var saas_users=parseInt($('#new_min_user').val());ajax.jsonRpc("/saas/trial/add/product",'call',{'product_id':product_id,'quantity':quantity,'saas_users':saas_users,}).then(function(a){location.href='/shop/cart';});});});;

/* /saas_kit_trial/static/src/js/saas_trial_contract_portal.js defined in bundle 'web.assets_frontend_lazy' */
odoo.define('odoo_saas_kit_trial.saas_trial_contract_portal',function(require){"user strict";var ajax=require('web.ajax');var contract_id=0;$('.pay_for_trial').click(function(ev){contract_id=$(event.target).attr('value');});$('#button_submit').click(function(ev){var radio_1=$('#radio_1').is(':checked');var radio_2=$('#radio_2').is(':checked');if(!contract_id){return}
if(radio_1){var new_contract=false;ajax.jsonRpc('/saa/trial/pay_now','call',{contract_id:parseInt(contract_id),from_trial:true,new_contract:new_contract,}).then(function(){location.href="/shop/cart";});}
else if(radio_2){var new_contract=true;ajax.jsonRpc('/saa/trial/pay_now','call',{contract_id:parseInt(contract_id),from_trial:true,new_contract:new_contract,}).then(function(){location.href="/shop/cart";});}});});;
